		...
		$.post('?m=controller&c=EsquadriaController', params, function( data ) {
			$.each( data, function( key, values ) {
		        ...
		        var values = {
			        ...
			        "PERFIL"	 : values.DESCRICAO_PERFIL + ' - ' + values.PESO_POR_METRO + ' g/m ',
	...
	buscarPerfil : function() {
		...
		$.post('?m=controller&c=EsquadriaController', parametros, function( data ) {
			...
			$.each(data, function (key, value) {
		 		options += '<option value="'+value.ID_PERFIL+'" >'+value.DESCRICAO+' - '+Money.formatBr(value.PESO_POR_METRO)+' g/m</option>';
		 	});
